...
module.exports = {
  configureWebpack: {
    // webpack 4 不支持 resolve.fallback，将 alias 移到这里
    resolve: {
      alias: {
        '@': resolve('src')
      }
    ...
    plugins: process.env.NODE_ENV === 'production' && semver.lt(process.version, '17.0.0') ? [
  ...
  devServer: {
    ...
    allowedHosts: ['0.0.0.0', 'localhost']  // 正确的配置数组格式
  },
  css: {
    loaderOptions: {
      sass: {
  ...
  chainWebpack(config) {
    // 为 webpack 4 配置 node polyfills（兼容 Node.js 18+ 和 20 版本）
